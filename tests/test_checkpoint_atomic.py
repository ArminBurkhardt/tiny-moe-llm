...
ponder_state = {"lambda_ponder": 0.18, "ema": 0.24, "last_adjust_tokens": 1_400_000_000}
...
epoch, idx, tokens, offset, losses, phase, ponder = load_checkpoint(m2, opt2, sched, path)
assert (epoch, idx, tokens, offset, phase) == (1, 7, 100_000_000, 4242, "phase1"), \
    (epoch, idx, tokens, offset, phase)
assert losses == [3.0, 2.0]
assert ponder == ponder_state, ponder
print("[ok] phase, global_offset and ponder_state round-trip")

# legacy checkpoint (no phase/ponder_state key) loads with both None rather than raising
...
epoch, idx, tokens, offset, losses, phase, ponder = load_checkpoint(m2, opt2, sched, legacy)
assert phase is None and tokens == 0 and offset == 0 and ponder is None, (phase, tokens, offset, ponder)
print("[ok] legacy checkpoints still load, phase and ponder_state are None")
